...
class DBManager:
    ...
    def get_all_vacancies(self):
        with self.conn.cursor() as cur:
            cur.execute(
                """SELECT c.company_name, v.title, v.salary_from, v.salary_to, v.currency, v.url FROM vacancies v INNER JOIN employers c ON v.employer_id = c.employer_id""")
            rows = cur.fetchall()
            for row in rows:
                print(row)
    ...
    def get_avg_salary(self):
        with self.conn.cursor() as cur:
            cur.execute(""" SELECT AVG((salary_from + salary_to)/2)::numeric(10,2) AS avg_salary 
            FROM vacancies WHERE vacancies.salary_from != 0.0 AND vacancies.salary_from != 0.0""")
    ...
    def get_vacancies_with_keyword(self, keyword):
        with self.conn.cursor() as cur:
            cur.execute(""" SELECT * FROM vacancies WHERE LOWER(title) LIKE %s """, ('%' + keyword.lower() + '%',))
            rows = cur.fetchall()
            for row in rows:
                print(f'{row}')